...
**Claim 2.1.** At MEDIUM privacy level (cover rate 1/30 Hz) with real message rate <= 1/30 Hz, the cover traffic makes active and idle periods `(0.78, 2^{-40})`-indistinguishable over any 120-second observation window within an epoch.

**Proof sketch.** Over a 120-second sub-window, the idle traffic follows `Poisson(4)`. Active traffic adds at most 4 real messages (matching the cover rate), giving a combined active trace of `Poisson(8)`. The adversary must distinguish `Poisson(4)` from `Poisson(8)` in a single window. The KL divergence is `KL(P_4 || P_8) = 4 ln(4/8) + (8 - 4) = 1.227`. By Pinsker's inequality, `TV <= sqrt(KL / 2) = sqrt(0.614) ~ 0.78`. The exact total variation distance (computed by direct summation) is approximately 0.59, so the Pinsker bound is conservative. Over longer observation periods spanning multiple sub-windows, accumulation is mitigated by the independent randomness of each Poisson interval. Note: the Dandelion++ epoch duration (600 seconds) governs relay peer rotation, not this indistinguishability window — the cover traffic rate (1/30 Hz) is continuous and independent of epoch boundaries.
