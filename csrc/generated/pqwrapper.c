...
uint32_t pqwrapper(const uint8_t* ek_recipient, const uint8_t* plaintext, const uint8_t* aad, const uint8_t* dk_recipient, const uint8_t* sealed_msg) {
    ...
    uint32_t shared_secret = 0; /* preprocessing: MLKEM768_Encaps(ek_recipient, m) */
    uint32_t ct_kem = 0; /* preprocessing: MLKEM768_Encaps(ek_recipient, m) */
    ...
    uint32_t ciphertext = 0; /* preprocessing: AES256GCM_Encrypt(aes_key, nonce, plaintext, aad) */
    uint32_t tag = 0; /* preprocessing: AES256GCM_Encrypt(aes_key, nonce, plaintext, aad) */
    uint32_t sealed = 0; /* preprocessing: ((((((ct_kem | 0) | nonce) | 0) | ciphertext) | 0) | tag) */
    uint32_t o_ct_kem = 0; /* preprocessing: sealed_msg[0] */
    uint32_t o_nonce = 0; /* preprocessing: sealed_msg[KEM_CT_LEN] */
    uint32_t o_ct_aead = 0; /* preprocessing: sealed_msg[KEM_CT_LEN] */
    ...
    uint32_t o_plaintext = 0; /* preprocessing: AES256GCM_Decrypt(o_aes_key, o_nonce, o_ct_aead, aad, o_tag) */
    uint32_t auth_ok = 0; /* preprocessing: AES256GCM_Decrypt(o_aes_key, o_nonce, o_ct_aead, aad, o_tag) */
