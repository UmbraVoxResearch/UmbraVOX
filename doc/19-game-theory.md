...
Treasury governance is fully specified in `doc/hardening/23-treasury-governance.md`, which defines:

- **Safety bounds**: `TREASURY_FLOOR = 55M MTK` (spending halts at floor), `MAX_TREASURY_SPEND_RATE = 5%` per cycle. At maximum spend with zero income, 58.4 cycles (642 days) to reach floor from cap.
- **Anti-capture protections**: no non-bonus disbursement may credit active/recent (3-cycle) validator addresses; beneficiaries determined by on-chain data only; no retroactive spending.
- **Multi-revision confirmation**: changes to safety bounds require the same change in 2 consecutive chain revisions ≥3 cycles apart.
- **Cartel resistance**: a malicious revision is publicly visible code; the 3-prior-revision support window allows honest validators to reject it and coordinate a response.
