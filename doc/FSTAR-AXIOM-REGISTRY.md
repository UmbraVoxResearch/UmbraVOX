...
5. **Arithmetic trust boundary** (1): `prime_is_prime` — primality of 2^255-19,
   externally verified by CAS and by deterministic Miller-Rabin primality
   certificate (see `test/evidence/formal-proofs/primality-certificate.txt`,
   generated by `scripts/primality-certificate.hs`). Infeasible to check
   via Z3 trial division.
