...
3. `authoritative-release` lane (QEMU)
- current scaffold validates host prerequisites for the planned QEMU
  authoritative lane.
...
4. `microvm-smoke` lane (QEMU)
- current entrypoint checks for a built Linux release artifact plus host
  prerequisites for QEMU.
- runner-hook execution is available via `UMBRAVOX_QEMU_SMOKE_RUNNER`.
- direct pinned-input boot paths are wired and invocable via
  `UMBRAVOX_QEMU_*` inputs.
- the QEMU path has a documented deterministic command-line profile helper
  for in-guest smoke intent.
...
- `./uv release smoke-qemu` is the microVM smoke entrypoint.
...
- pinned microVM runs now require an explicit in-guest verification command
  template via `UMBRAVOX_QEMU_VERIFY_CMD`; the repo still does not ship the
  maintained guest image that executes that command by default.
- `./uv release lane-qemu` remains a host-prerequisite scaffold only.
...
- microVM readiness baseline: `./uv release smoke-qemu` verifies artifact
  presence, QEMU availability, and `/dev/kvm` before any runner hook or
  pinned boot path.
- microVM direct invocation readiness: QEMU can execute caller-supplied
  runner hooks or pinned boot inputs when those inputs are provided
  explicitly.
...
- no authoritative QEMU in-guest release graph currently produces the final
  Linux artifact inside the guest
...
| --- | --- | --- | --- |
| Linux x86_64 | `linux-hosted-fast` now; `linux-kvm-integration` / `linux-kvm-release` later | Native host packaging works now; QEMU lane entrypoints are scaffolded only | Native Linux terminal bundle is the only current prebuilt artifact |
...
- The future authoritative Linux lane is the in-guest QEMU release path
  described above; that execution model is not complete yet.
...
- `./uv release lane-qemu` only verifies host prerequisites and prints the
...
- `./uv vm smoke release qemu` verifies artifact + host prerequisites
  and can then either:
  run a host-provided runner hook via `UMBRAVOX_QEMU_SMOKE_RUNNER`, or
  invoke a direct pinned-input boot path for QEMU.
...
- The default path is still messaging plus prerequisite checks unless one of
  those execution hooks/inputs is explicitly provided.
