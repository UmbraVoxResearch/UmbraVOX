...
lane_finish \
  ...
  "run ./uv release windows-cli from the Windows Bash environment for the current source artifact" \
  "run ./uv build and ./uv test on the same host if you want native CLI parity evidence" \
