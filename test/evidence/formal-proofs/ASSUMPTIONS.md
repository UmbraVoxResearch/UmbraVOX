...
| ED-006 | Spec.Ed25519 | `scalar_mod_L_equiv` | DERIVED_FROM_ALGEBRA | no | ED-002, ED-004 | Requires group_order_lemma + scalar_mult_add | Standard group theory | Prove from group_order_lemma (EXTERNALLY_VERIFIED) + ED-004 (UNBLOCKED) | UNBLOCKED |
...
| X2-006 | Spec.X25519 | `dh_commutativity_general` | DERIVED_FROM_ALGEBRA | no | X2-005, ED-005 | Follows algebraically from group law + scalar_mult_compose (ED-005, DISCHARGED); not a tooling limitation | Elliptic curve group law | Prove from scalar_mult_compose (ED-005 UNBLOCKED) | DERIVED_FROM_ALGEBRA |
...
| PQ-001 | Spec.PQWrapper | `ind_cca2_security` | CRYPTO_HARDNESS | yes | none | KEM/DEM composition (ML-KEM-768 + AES-256-GCM + HKDF) yields IND-CCA2. **COVERAGE GAP: Spec.MLKEM768.fst is STUBS ONLY as of v0.7.0. All core functions are identity stubs: `let ntt f = f`, `let inv_ntt f_hat = f_hat`, `let mlkem_keygen seed = (seed, seed)`, `let mlkem_encaps ek m = (m, m)`, `let mlkem_decaps dk ct = ct`. The IND-CCA2 composition (PQ-001) cannot be meaningfully audited against stubs. PREREQUISITE: Spec.MLKEM768.fst must be fully specified (M37) before this assumption can be audited, and M36B.11 must deliver Low* extraction before production C is formally connected to this spec.** | FIPS 203; Cramer-Shoup 2003 | Cannot be proved unconditionally | CRYPTO_HARDNESS |
...
| DERIVED_FROM_ALGEBRA | 8 | ED-004, ED-005, ED-006, ED-008a, ED-008d, VR-001, X2-005, X2-006 |
| BLOCKED_BY_TOOLING | 0 | (none — ED-002 reclassified EXTERNALLY_VERIFIED; X2-006 reclassified DERIVED_FROM_ALGEBRA) |
