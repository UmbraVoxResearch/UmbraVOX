...
| ED-009 | Spec.Ed25519 | `encode_decode_round_trip` | ALGEBRAIC_EXTERNAL | yes | ED-001 (prime) | Theorem false as stated: missing on_curve_ext precondition. Off-curve points fail decode when x=0 and sign bit set. Needs precondition fix + sqrt recovery proof (~100 lines). | Standard elliptic curve encoding | Fix precondition, then prove with Euler criterion + Fermat | NEEDS_PRECONDITION_FIX |
