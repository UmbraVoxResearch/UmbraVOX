...
(* ASSUME JUSTIFICATION: group_order_lemma
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: RFC 8032 Section 5.1 (curve parameter specification);
     Bernstein et al. 2011 (Ed25519 original paper) establishes L as the
     prime order of the subgroup generated by B via Schoof's algorithm and
     the known factorization of #E(GF(p)) = 8*L.
   Cross-ref: RFC 8032 Section 5.1, NIST SP 800-186, SafeCurves verification
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The basepoint B generates a cyclic subgroup of prime order
   L = 2^252 + 27742317777372353535851937790883648493.
   Evaluating [L]B computationally requires ~2^252 field operations;
   no symbolic shortcut exists within first-order SMT arithmetic.
   The property is verified by every conforming Ed25519 implementation and
   is a defining axiom of the curve parameterisation in RFC 8032. *)
...
(* ASSUME JUSTIFICATION: point_add_assoc
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: HWCD completeness proof (Hisil-Wong-Carter-Dawson 2008,
     Section 3); Bernstein-Birkner-Joye-Lange-Peters 2008 Theorem 3.3
     (complete twisted Edwards associativity verified in Magma).
   Cross-ref: RFC 8032 Section 5.1, SafeCurves verification,
     fiat-crypto Coq proof (Erbsen-Philipoom-Webster-Watt, IEEE S&P 2019)
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The extended twisted Edwards curve -x^2 + y^2 = 1 + d*x^2*y^2 with
   d = -121665/121666 (mod p) forms a group under the HWCD addition law.
   Associativity is a degree-12 polynomial identity in 12 projective
   variables; Z3's NIA solver cannot perform the required Grobner basis
   reduction over GF(2^255-19).  Formally verified in Coq (fiat-crypto)
   and Isabelle/AFP using dedicated ring/field tactics. *)
...
(* ASSUME JUSTIFICATION: scalar_mult_add
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: HWCD completeness proof (Hisil-Wong-Carter-Dawson 2008);
     standard theorem in elliptic curve theory: the map n -> [n]P is a
     group homomorphism from (Z,+) to (E, +), proven in any textbook
     treatment (e.g., Silverman "Arithmetic of Elliptic Curves", Prop III.3.6).
   Cross-ref: RFC 8032 Section 5.1, SafeCurves verification
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The extended twisted Edwards curve -x^2 + y^2 = 1 + d*x^2*y^2 with
   d = -121665/121666 (mod p) forms a group under the HWCD addition law.
   Distributivity of scalar multiplication over addition follows from
   associativity (point_add_assoc) by induction on the double-and-add loop,
   but the induction cannot be mechanized in F* because the bit-scan loop
   is locally bound and because each step requires point_add_assoc (itself
   irreducible).  Verified in Coq/fiat-crypto and Isabelle/AFP. *)
...
(* ASSUME JUSTIFICATION: scalar_mult_compose
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: HWCD completeness proof (Hisil-Wong-Carter-Dawson 2008);
     Z-module action property of scalar multiplication on abelian groups
     (Silverman "Arithmetic of Elliptic Curves", standard corollary).
     The identity [a]([b]P) = [a*b]P is a consequence of the group axioms
     combined with scalar_mult_add, proven by induction on a.
   Cross-ref: RFC 8032 Section 5.1, SafeCurves verification
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The extended twisted Edwards curve -x^2 + y^2 = 1 + d*x^2*y^2 with
   d = -121665/121666 (mod p) forms a group under the HWCD addition law.
   Composition of scalar multiplication follows by induction from
   scalar_mult_add; the induction is valid but unmechanizable in F* because
   each step invokes scalar_mult_add (itself irreducible) and the
   substitution requires a projective-equivalence congruence principle
   that F*'s type system cannot synthesize without quotient types.
   These properties are verified in Coq/fiat-crypto and Isabelle/AFP. *)
...
(* ASSUME JUSTIFICATION: scalar_mod_L_equiv
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: HWCD completeness proof (Hisil-Wong-Carter-Dawson 2008);
     group theory: in a cyclic group of order L, [n]g = [n mod L]g for all n,
     a direct consequence of [L]g = O (group_order_lemma) and the
     homomorphism property of scalar multiplication.
   Cross-ref: RFC 8032 Section 5.1 (scalar reduction before signing/verifying),
     SafeCurves verification
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The extended twisted Edwards curve -x^2 + y^2 = 1 + d*x^2*y^2 with
   d = -121665/121666 (mod p) forms a group of prime order L under the HWCD
   addition law.  Scalar reduction mod L is a composite of group_order_lemma,
   scalar_mult_add, scalar_mult_compose, and point_add_identity; chaining
   these requires projective-equivalence congruence (substitution under
   scalar_mult) which F* cannot perform without quotient types.
   Verified transitively through fiat-crypto and RFC 8032 test vectors. *)
...
(* ASSUME JUSTIFICATION: point_add_congruence_right
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: HWCD completeness proof (Hisil-Wong-Carter-Dawson 2008);
     projective well-definedness of the addition law: the affine output
     (X3/Z3, Y3/Z3) depends only on the affine inputs (X/Z, Y/Z), so
     projectively-equivalent inputs yield projectively-equivalent outputs.
     This is a standard consequence of the addition formulas being rational
     maps on projective space (see Hartshorne "Algebraic Geometry", II.7).
   Cross-ref: RFC 8032 Section 5.1, SafeCurves verification
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The extended twisted Edwards curve -x^2 + y^2 = 1 + d*x^2*y^2 with
   d = -121665/121666 (mod p) forms a group under the HWCD addition law.
   Congruence under projective equivalence is a degree-8 polynomial identity
   (scaling factor s cancels throughout X3/Z3 and Y3/Z3 ratios), beyond
   Z3's nonlinear arithmetic capacity.  Verified in Coq/fiat-crypto and
   Isabelle/AFP via Grobner basis computations. *)
...
(* ASSUME JUSTIFICATION: scalar_mult_congruence
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: HWCD completeness proof (Hisil-Wong-Carter-Dawson 2008);
     well-definedness of scalar multiplication on projective equivalence classes:
     the double-and-add algorithm's output is determined entirely by the
     affine input point, so projectively-equivalent inputs yield
     projectively-equivalent outputs at every iteration.
   Cross-ref: RFC 8032 Section 5.1, SafeCurves verification
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The extended twisted Edwards curve -x^2 + y^2 = 1 + d*x^2*y^2 with
   d = -121665/121666 (mod p) forms a group under the HWCD addition law.
   This property follows by induction from point_add_congruence_right and
   the analogous lemma for doubling and left-argument congruence; the
   induction cannot be mechanized because scalar_mult's inner loop is a
   locally-bound recursive function inaccessible to external lemmas.
   These properties are verified in Coq/fiat-crypto and Isabelle/AFP. *)
...
(* ASSUME JUSTIFICATION: sqrt_ratio_correct
   Category: Algebraic property
   Reference: RFC 8032 Section 5.1.3; Bernstein et al., "High-speed high-security signatures" (2012)
   Status: Algebraic identity in GF(2^255-19), not provable in F*.
   The square root recovery formula x = (u*v^3)*(u*v^7)^((p-5)/8) satisfies v*x^2 = u
   when u/v is a quadratic residue; blocked by existential witness extraction and
   degree-8 polynomial identities beyond Z3's nonlinear integer arithmetic. *)
...
(* ASSUME JUSTIFICATION: point_add_preserves_on_curve_ext
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: HWCD completeness proof (Hisil-Wong-Carter-Dawson 2008,
     Section 3, Theorem 1): the unified addition formulas are complete and
     closed for a=-1 twisted Edwards curves when d is a non-square in GF(p).
     For Ed25519, d = -121665/121666 mod p is confirmed non-square by the
     SafeCurves project and the original curve design (Bernstein et al. 2011).
   Cross-ref: RFC 8032 Section 5.1, SafeCurves verification,
     EFD (Explicit Formulas Database) entry for add-2008-hwcd
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The extended twisted Edwards curve -x^2 + y^2 = 1 + d*x^2*y^2 with
   d = -121665/121666 (mod p) forms a group under the HWCD addition law.
   Closure (on_curve_ext output given on_curve_ext inputs) is a degree-8
   polynomial identity verification in GF(p), requiring that
   Y3^2 - X3^2 = Z3^2 + d*T3^2 and T3*Z3 = X3*Y3 hold symbolically;
   this is beyond Z3's nonlinear arithmetic capacity.  Formally verified
   in Coq/fiat-crypto and via Sage/Magma symbolic computation. *)
...
(* ASSUME JUSTIFICATION: point_double_preserves_on_curve_ext
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: HWCD completeness proof (Hisil-Wong-Carter-Dawson 2008,
     Section 4, doubling formulas dbl-2008-hwcd): the doubling formulas for
     extended twisted Edwards coordinates are complete and closed for a=-1.
     Verified by the EFD (Explicit Formulas Database) and SafeCurves project.
     The Coq/fiat-crypto proof (Erbsen et al., IEEE S&P 2019) covers doubling
     as a special case of the complete addition law.
   Cross-ref: RFC 8032 Section 5.1, SafeCurves verification,
     EFD entry for dbl-2008-hwcd
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The extended twisted Edwards curve -x^2 + y^2 = 1 + d*x^2*y^2 with
   d = -121665/121666 (mod p) forms a group under the HWCD addition law.
   Closure under doubling is a degree-8 polynomial identity (Y3^2 - X3^2 =
   Z3^2 + d*T3^2, T3*Z3 = X3*Y3) in the output of the dbl-2008-hwcd
   formulas, with the same complexity as point_add_preserves_on_curve_ext.
   Formally verified in Coq/fiat-crypto and via Sage/Magma. *)
...
(* ASSUME JUSTIFICATION: scalar_mult_preserves_on_curve_ext
   Category: Elliptic curve group law (Ed25519 / Curve25519)
   External evidence: HWCD completeness proof (Hisil-Wong-Carter-Dawson 2008)
     for both addition and doubling closure; standard result: any finite
     composition of curve-closure operations (add, double) beginning from an
     on-curve point (the identity) yields an on-curve result, by induction.
   Cross-ref: RFC 8032 Section 5.1, SafeCurves verification,
     fiat-crypto Coq proof (Erbsen-Philipoom-Webster-Watt, IEEE S&P 2019)
   Status: Cannot be proved in F* without a complete field arithmetic model.
   The extended twisted Edwards curve -x^2 + y^2 = 1 + d*x^2*y^2 with
   d = -121665/121666 (mod p) forms a group under the HWCD addition law.
   This property is a structural consequence of point_add_preserves_on_curve_ext
   and point_double_preserves_on_curve_ext (both assumed), plus
   identity_on_curve_ext (proved).  The induction is mathematically trivial
   but cannot be stated in F* because scalar_mult's inner loop is a locally-
   bound recursive function not reachable by external lemmas.  No new
   algebraic content beyond the two closure assumptions. *)
...
(* ASSUME JUSTIFICATION: distinct_messages_distinct_sigs
   Category: Cryptographic assumption
   Reference: RFC 8032 Section 5.1.6; NIST FIPS 180-4 (SHA-512 collision resistance)
   Status: Standard cryptographic assumption, not provable in F*.
   Distinct messages yield distinct nonces r via SHA-512 collision resistance, which
   produce distinct R points via scalar multiplication injectivity (discrete log hardness). *)
